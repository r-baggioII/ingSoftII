...
export function LibroForm({
  ...
  const renderSpecificField = (tipo: TipoLibro) => {
    if (tipo === 'FISICO') {
      return (
        <div className="form-row">
          <label htmlFor="pesoGramos">Peso (g)</label>
          <input
            id="pesoGramos"
            type="number"
            value={form.pesoGramos ?? 0}
            onChange={(e) =>
              setForm((prev) => ({
                ...prev,
          ...
          />
          {errors.pesoGramos && <small className="error">{errors.pesoGramos}</small>}
        </div>
      );
    }
    return (
      <div className="form-row">
        <label htmlFor="tamanoMb">Tamaño (MB)</label>
        <input
          id="tamanoMb"
          type="number"
          value={form.tamanoMb ?? 0}
          onChange={(e) =>
            setForm((prev) => ({
              ...prev,
        ...
        />
        {errors.tamanoMb && <small className="error">{errors.tamanoMb}</small>}
      </div>
    );
  };
    ...
    <form onSubmit={handleSubmit} className="card">
      ...
      <div className="form-row">
        ...
        <select
          ...
          value={form.tipo}
          onChange={(e) => {
            const nuevoTipo = e.target.value as TipoLibro;
            setForm((prev) => ({
              ...prev,
              tipo: nuevoTipo,
