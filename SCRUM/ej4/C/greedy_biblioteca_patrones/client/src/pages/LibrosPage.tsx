...
export function LibrosPage() {
  ...
  const [filters, setFilters] = useState<LibroFilters>({});
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [current, setCurrent] = useState<LibroDTO>(emptyLibro);
  const [iteradorLibros, setIteradorLibros] = useState<LibroDTO[] | null>(null);
    ...
    <section>
      ...
      <div className="card" style={{ marginTop: '1.5rem' }}>
        <div className="filters">
          ...
          <div className="form-row">
            <label htmlFor="f-criterio">Criterio</label>
            <select
              id="f-criterio"
              ...
              onChange={(e) =>
                setFilters((prev) => ({
                  ...prev,
                  criterio: e.target.value ? (e.target.value as LibroFilters['criterio']) : undefined
                }))
              }
            >
              <option value="">Todos</option>
          ...
          <div className="form-row">
            <label>&nbsp;</label>
            <button type="button" className="secondary" onClick={handleIterador}>
              Iterador por autor
            </button>
          </div>
        </div>
            ...
            <table>
              ...
              <tbody>
                {libros.content.map((libro) => (
                  <tr key={libro.id}>
                    <td>{libro.titulo}</td>
                    <td>{libro.tipo === 'FISICO' ? 'Físico' : 'Digital'}</td>
                    <td>
                      {libro.tipo === 'FISICO'
                        ? `${libro.pesoGramos ?? '-'} g`
                        : `${libro.tamanoMb ?? '-'} MB`}
      ...
      {iteradorLibros && (
        <div className="card" style={{ marginTop: '1rem' }}>
          <h3>Resultado Iterador Autor</h3>
          {iteradorLibros.length === 0 ? (
            <p>No se encontraron libros para el autor seleccionado.</p>
          ) : (
            <ul>
              {iteradorLibros.map((libro) => (
                <li key={`iterador-${libro.id ?? libro.titulo}`}>
                  {libro.titulo} — {libro.tipo === 'FISICO' ? `${libro.pesoGramos} g` : `${libro.tamanoMb} MB`}
                </li>
              ))}
            </ul>
          )}
