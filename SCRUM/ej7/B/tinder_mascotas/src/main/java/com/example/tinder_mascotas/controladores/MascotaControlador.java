...
public class MascotaControlador {
    ...
    public String misMascotas(Model model, Principal principal) {
        try {
            ...
            com.example.tinder_mascotas.entidades.Usuario u = usuarioRepositorio.buscarPorCorreo(principal.getName());
    ...
    public String mascotasDeBaja(Model model, Principal principal) {
        try {
            if (principal == null) { return "redirect:/login"; }
            com.example.tinder_mascotas.entidades.Usuario u = usuarioRepositorio.buscarPorCorreo(principal.getName());
            List<Mascota> lista = mascotaRepositorio.buscarPorUsuario(u.getId()).stream()
    ...
    public String eliminarPerfil(@RequestParam("id") String idMascota, Principal principal, Model model) {
        if (principal == null) { return "redirect:/login"; }
        com.example.tinder_mascotas.entidades.Usuario u = usuarioRepositorio.buscarPorCorreo(principal.getName());
        try {
    ...
    public String altaPerfil(@RequestParam("id") String idMascota, Principal principal, Model model) {
        if (principal == null) { return "redirect:/login"; }
        com.example.tinder_mascotas.entidades.Usuario u = usuarioRepositorio.buscarPorCorreo(principal.getName());
        try {
