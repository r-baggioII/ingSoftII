...
import { toPersonaPayload, type PersonaPayload } from './payloads';
...
export async function createPersona(persona: PersonaDTO): Promise<PersonaDTO> {
  return apiPost<PersonaDTO, PersonaPayload>('/personas', toPersonaPayload(persona));
}

export async function updatePersona(id: number, persona: PersonaDTO): Promise<PersonaDTO> {
  return apiPut<PersonaDTO, PersonaPayload>(`/personas/${id}`, toPersonaPayload(persona));
}
